...
function getOrInitializeResumeData() {
    ...
    if (resumeDataString) {
        ...
        console.log("No resume data found in localStorage. Predefined data has been saved.");
        return predefinedResumeData;
    }
}
// Function to populate the UI with resume data
function populateUI(resumeData) {
    // Basic Info
    ...
    if (nameElement)
        nameElement.textContent = resumeData.basicInfo.name;
    ...
    if (addressElement)
        addressElement.textContent = resumeData.basicInfo.address;
    // Socials
    ...
    resumeData.basicInfo.socials.forEach((social, index) => {
        if (socialItems[index]) {
            ...
            if (socialLink)
                socialLink.setAttribute("href", social.url);
            if (socialPlatform)
                socialPlatform.textContent = social.platform;
            if (socialUsername)
                socialUsername.textContent = `@${social.username}`;
        }
    });
    // Education
